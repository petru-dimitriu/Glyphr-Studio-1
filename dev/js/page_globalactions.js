...
function loadPage_globalactions(){
	...
	// Auto width and fit 
	con += '<h1>Auto Width and Fit All</h1>';
	con += 'Move all glyphs near the left margin ( x = 0 ) and turn on auto width in order to perfectly fit ' +
			'them inside. ';
	con += '<button class="buttonsel commit" onclick="autoWidenAndFit();">Turn on auto width and fit</button>';
	con += '<hr>';
...
function autoWidenAndFit() {

	glyphIterator({
		title: 'Turning on auto width and fitting inside',
		filter: function(glyphid){ 
			var gtype = getGlyphType(glyphid);
			return gtype === 'glyph' || gtype === 'ligature';
		},
		action: function(glyph){
			glyph.isautowide = true;
			glyph.setGlyphPosition(0, glyph.value, true);
		}
	});
}
